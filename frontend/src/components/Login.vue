    ...
    <div class="absolute inset-0 flex items-start md:items-center md:justify-center md:p-4">
      ...
      >
        ...
        <div class="px-6 pb-10 pt-4 md:p-8 md:pt-10 lg:p-10">
          ...
          <!-- Opciones de Login (solo si no hay flujo activo) -->
          <div v-if="codeStep === 0 && !showAdminForm" class="space-y-3 max-w-[340px] mx-auto">
            ...
            <button 
              type="button"
              @click="startCodeFlow"
              class="w-full h-10 flex items-center px-3 bg-white border border-[#dadce0] rounded hover:bg-[#f8f9fa] hover:border-[#d2e3fc] active:bg-[#f1f3f4] transition-colors"
              style="font-family: 'Google Sans', Roboto, arial, sans-serif;"
            >
              <div class="w-[18px] h-[18px] flex items-center justify-center mr-3">
                <svg class="w-[18px] h-[18px] text-[#5f6368]" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                </svg>
              </div>
              <span class="flex-1 text-center text-[14px] font-medium text-[#3c4043] pr-[30px]">Ingresar con código de acceso</span>
            </button>
          ...
          >
            <div v-if="codeStep > 0" class="space-y-4">
              
              <!-- Paso 1: Ingresar email -->
              <form v-if="codeStep === 1" @submit.prevent="sendLoginCode" class="space-y-4">
                ...
                >
                <div v-if="codeError" class="text-red-500 text-sm text-center">{{ codeError }}</div>
                <div v-if="codeSuccess" class="text-green-600 text-sm text-center">{{ codeSuccess }}</div>
                <button 
                  type="submit"
                ...
                >
                  <span v-if="!codeLoading">Enviar código</span>
                ...
                <p class="text-center">
                  <button type="button" @click="cancelCodeFlow" class="text-sm text-text-light hover:text-brand-600 transition-colors">
                    ← Volver
                  </button>
                </p>
              </form>
              ...
              <form v-if="codeStep === 2" @submit.prevent="verifyLoginCode" class="space-y-4">
                <p class="text-sm text-text-medium text-center">
                  Ingresa el código que enviamos a <strong>{{ codeForm.email }}</strong>
                </p>
                <input 
                  type="text" 
                ...
                <button 
                  ...
                  class="w-full bg-brand-600 hover:bg-brand-700 text-white font-semibold py-4 px-6 rounded-xl transition-all disabled:opacity-70"
                >
                  <span v-if="!codeLoading">Verificar código</span>
                  <span v-else class="flex items-center justify-center gap-2">
                    <svg class="animate-spin w-5 h-5" fill="none" viewBox="0 0 24 24">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                ...
                <p class="text-center">
                  <button type="button" @click="codeStep = 1" class="text-sm text-text-light hover:text-brand-600 transition-colors">
                    ← Reenviar código
                  </button>
                </p>
              </form>
          ...
          <!-- ===== FORMULARIO ADMIN (email + contraseña) ===== -->
          <transition
            enter-active-class="transition duration-300 ease-out"
            enter-from-class="opacity-0 -translate-y-2"
            enter-to-class="opacity-100 translate-y-0"
            ...
            leave-to-class="opacity-0 -translate-y-2"
          >
            <form v-if="showAdminForm" @submit.prevent="handleLogin()" class="space-y-4">
              <div class="flex items-center gap-2 justify-center mb-4">
                <svg class="w-5 h-5 text-text-light" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
                </svg>
                <span class="text-sm font-medium text-text-medium uppercase tracking-wider">Acceso Administrador</span>
              </div>
              ...
              <p class="text-center">
                <button type="button" @click="showAdminForm = false" class="text-sm text-text-light hover:text-brand-600 transition-colors">
                  ← Volver
                </button>
              </p>
            </form>
          ...
          <!-- Divider (solo cuando no hay flujo activo) -->
          <div v-if="codeStep === 0 && !showAdminForm" class="my-10 md:my-8 border-t border-nude-200"></div>

          <!-- Link Admin (solo cuando no hay flujo activo) -->
          <p v-if="codeStep === 0 && !showAdminForm" class="text-center text-sm text-text-light">
            <button @click="showAdminForm = true" class="hover:text-text-dark transition-colors">
              ¿Eres admin?
            </button>
          </p>
...
export default {
  ...
  setup() {
    ...
    const codeStep = ref(0) // 0=oculto, 1=email, 2=código
    ...
    // Forgot password state (para admins)
    ...
    const sendLoginCode = async () => {
      ...
      try {
        const response = await apiClient.post('/auth/send-login-code', {
          email: codeForm.email
        })
        
        if (response.data.sent) {
          codeSuccess.value = 'Te enviamos un código a tu correo'
          setTimeout(() => {
            codeSuccess.value = ''
            codeStep.value = 2 // Pasar a ingresar código
          }, 1500)
        } else {
          codeError.value = 'No pudimos enviar el código. Intenta de nuevo.'
        }
      } catch (err) {
        console.error('Error enviando código:', err)
        codeError.value = err.response?.data?.detail || 'Error al enviar el código'
      } finally {
    ...
    const verifyLoginCode = async () => {
      ...
      try {
        ...
        codeError.value = err.response?.data?.detail || 'Código inválido o expirado'
      } finally {
        codeLoading.value = false
      }
    }
