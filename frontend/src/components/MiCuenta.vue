        ...
        <nav class="flex gap-6">
          <button 
            @click="activeTab = 'favoritos'"
              class="pb-3 text-xs tracking-[0.15em] uppercase transition-colors relative"
      ...
      <section v-if="activeTab === 'compras'">
        ...
        <div v-else class="space-y-3">
          <button 
            ...
            class="w-full text-left bg-white border border-gray-200 rounded-xl overflow-hidden hover:border-gray-300 hover:shadow-md active:bg-gray-50 active:scale-[0.995] transition-all duration-200 cursor-pointer group"
          >
            ...
            <div class="px-4 py-3 flex items-center gap-3">
              ...
              <!-- Chevron indicator -->
              <svg class="w-5 h-5 text-gray-300 flex-shrink-0 group-hover:text-gray-500 group-hover:translate-x-0.5 transition-all duration-200" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
            ...
            <div class="px-4 py-2.5 border-t border-gray-100 flex items-center justify-between text-[11px] text-gray-400">
              <span class="font-medium">Pedido #{{ orden.numero }}</span>
              <span v-if="orden.estado_pago === 'pagado'" class="flex items-center gap-1 text-green-600">
                <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                </svg>
                Pagado
              </span>
              <span v-else class="text-amber-500 font-medium">Pendiente de pago</span>
            </div>
          </button>
        </div>
      </section>
        ...
        >
          ...
          <div class="px-5 py-4 border-t border-gray-100 bg-white">
            ...
            <a 
              ...
              class="w-full h-11 inline-flex items-center justify-center gap-2 bg-[#D81B60] text-white text-xs font-semibold rounded-lg hover:bg-[#C2185B] active:scale-[0.98] transition-all mb-2"
            >
            ...
            <button 
              ...
              class="w-full h-11 inline-flex items-center justify-center gap-2 bg-white border border-gray-300 text-gray-700 text-xs font-medium rounded-lg hover:bg-gray-50 hover:border-gray-400 active:scale-[0.98] transition-all mb-2"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <!-- Botón terciario: Contactar soporte (discreto) -->
            <a 
              :href="getWhatsAppSupportUrl(selectedOrder)"
              target="_blank"
              rel="noopener noreferrer"
              class="w-full h-10 inline-flex items-center justify-center gap-2 text-gray-500 text-xs hover:text-gray-700 transition-colors"
            >
...
export default {
  ...
  setup() {
    ...
    // Obtener URL de WhatsApp para soporte sobre el pedido
    const getWhatsAppSupportUrl = (orden) => {
      const whatsappNumber = '4796657763'
      const mensaje = encodeURIComponent(
        `Hola, tengo una consulta sobre mi pedido #${orden?.numero || ''}. ¿Podrían ayudarme?`
      )
